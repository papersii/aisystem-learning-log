# GPU 学习记录：写 kernel + 论文笔记

我是刚开始学 CUDA / ML systems 的本科生。这个仓库记录我的两条学习线：**动手线**——从零手写 SGEMM（fp32 矩阵乘法）并一步步优化，所有数字在云端的 RTX 4090 上实测；**论文线**——围绕"kernel 工程自动化"这个主题读论文、记笔记（Mirage、MPK 等）。
...
三个工作是可以串在一起的，Mirage解决的是单个Kernel的构造和优化，PipeThreader解决Kernel内部的流水线排布（以前负责这部分的硬件调度器，无法应付复杂程度高的Kernel），MPK则是更进一步，移除了每个Kernel的边界，将它们融合成一个巨大的megakernel。三者串在一起，把优化的决策权逐步从人工和硬件中搬到了编译器上。因为前段时间初步学习时手写和优化过Kernel，我能深刻体会到三者带来的提效和便利。
